...
1. 下载并运行本程序（Windows 用户可直接运行 .exe，macOS / Linux 需安装 Python 依赖）  
   1. Download and run the program (Windows users can directly run the .exe; macOS / Linux requires Python dependencies)  
2. 选择或确认下载目录（默认为系统“下载”文件夹）  
   2. Select or confirm the download directory (default is your system’s "Downloads" folder)  
3. 从版本列表中勾选需要下载的版本，也可使用搜索快速定位  
   3. Check the versions you want from the list; use the search box to quickly locate a specific version  
4. 点击“开始下载”，即可自动下载对应版本 JAR 文件  
   4. Click “Start Download” and the corresponding JAR files will be downloaded automatically  
5. 下载完成后，JAR 文件会按版本号分组存放在子文件夹中，可直接用于启动服务器  
   5. After download, JAR files are grouped in sub‑folders by version and can be used directly to launch your server  
...
如果你遇到任何问题，或有新功能建议，欢迎提交 Issue 或 Pull Request！  
If you encounter any problems or have feature suggestions, feel free to submit an Issue or Pull Request!  

---

## ⭐ 求 Star 支持 | Request Star Support  
如果这个工具帮你省去了找服务器核心的麻烦，不妨点亮右上角的 **Star** 🌟 ！你的支持是我持续改进的最大动力，也能让更多 Minecraft 玩家发现它～  
If this tool saved you the hassle of hunting for server cores, please light up the **Star** 🌟 in the upper right corner! Your support is my biggest motivation to keep improving, and it will help more Minecraft players discover this useful tool!  
...
- [你的名字 / 你的 GitHub 用户名]：项目发起者，负责核心功能开发与维护  
  - [Your Name / Your GitHub Username]: Project initiator, responsible for core development and maintenance  
...
💡 持续关注仓库，获取最新版本更新通知！  
💡 Follow the repository to get notifications about the latest updates!  
